...
export default function InvestorDashboard() {
    ...
    return (
        <div className={`min-h-screen ${isDark ? "bg-[#030711] text-white" : "bg-white text-gray-900"}`}>
            {/* Sidebar */}
            {!hideNav && (
                <Sidebar
            ...
            {/* Main area */}
            <div className={`${!hideNav ? "lg:ml-64" : ""} flex flex-col min-h-screen transition-all`}>
                {!hideNav && (
                    <Topbar
                        setSidebarOpen={setSidebarOpen}
                        icons={icons}
            ...
            {hideNav && (
                <button
                    ...
                    className="fixed top-24 left-0 z-50 bg-indigo-600/90 backdrop-blur-md text-white rounded-r-xl p-3 shadow-xl hover:bg-indigo-700 transition-all transform hover:translate-x-1"
                    title="Show Menu"
                >
            ...
            {!hideNav && (
            <button
                onClick={() => toast.info("Scheduler feature coming soon!")}
